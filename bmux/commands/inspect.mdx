...
It writes nothing and changes nothing. Images and audio files (mp3/flac/m4a) are both handled.
...
<ResponseField name="audio" type="object">
  Present only if the file could be read as mp3, flac, or m4a.
  ...
  <Expandable title="audio">
    ...
    <ResponseField name="tags" type="object">
      `title`, `artist`, `album`, `trackNumber`, `trackTotal`, `year` — each omitted if unset.
    </ResponseField>

    <ResponseField name="hasCoverArt" type="boolean">
      Whether the file has embedded cover art.
    </ResponseField>

    <ResponseField name="coverArtDimensions" type="string">
      `"widthxheight"`, when the container records it (flac does; id3/mp4 don't).
    </ResponseField>
  </Expandable>
</ResponseField>
