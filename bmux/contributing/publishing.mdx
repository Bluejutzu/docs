...
<Note>
  OIDC trusted publishing is an **npm CLI** feature (npm ≥11.5.1) — pnpm's `publish` command has
  no OIDC support at all as of the 10.x line this repo pins. The workflow's "Publish to npm" step
  upgrades npm and runs `npm publish` directly from `packages/cli` for this reason, rather than
  `pnpm --filter @bluejutzu/bmux publish`. It still builds against the `node_modules` layout pnpm
  installed earlier in the job, so `@bmux/core` resolves the same way either tool runs it.
</Note>
