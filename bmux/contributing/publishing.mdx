...
<Steps>
  <Step title="Create the @bmux org">
    Both packages publish under the `@bmux` scope — `bmux` alone is blocked by npm's anti-squat
    check (too close to existing names like `bun` and `flux`), and a new scope has to exist as an
    org before npm will accept a publish under it. Create it for free at
    [npmjs.com/org/create](https://www.npmjs.com/org/create).
  </Step>

  <Step title="Publish the first version by hand">
    From a clean checkout, logged in to an account that's a member of the `@bmux` org:
    ...
    pnpm --filter @bmux/cli publish --access public --no-git-checks
    ```

    `core` goes first — the CLI depends on it via `workspace:*`, and pnpm rewrites that to the exact
    version being published, so `core` has to exist on the registry before the CLI does.
  ...
  <Step title="Configure the trusted publisher">
    For **each** package (`@bmux/core` and `@bmux/cli`), on npmjs.com: package page → **Settings** →
